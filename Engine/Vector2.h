...
namespace nu
{
	struct Vector2
	{
		...
		float LengthSqr() const { return (x * x) + (y * y); } //Vec3
		float Length() const { return std::sqrt((x * x) + (y * y)); } //Vec3
		Vector2 Normalized() const { return (*this) / Length(); } //Vec3
		float Dot(const Vector2& v) const { return (this->x * v.x) + (this->y * v.y); } //Vec3
		float Angle() const { return std::atan2(this->y, this->x); }
		float AngleBetween(const Vector2& v) const { return std::acos(Dot(v)); }

		Vector2 Rotate(float rads)
		{
			Vector2 v;

			v.x = this -> x * std::cos(rads) - this -> y * std::sin(rads);
			v.y = this -> x * std::sin(rads) + this->y * std::cos(rads);
